...
class ConstantScanner {
  ...
  formatResults(results, options = {}) {
    ...
    let output = '';
    const { verbose = false, showPath = true } = options;

    output += `找到 ${results.length} 个常量类:\n\n`;

    results.forEach((item, index) => {
      const prefix = item.isInCommon ? '[common] ' : '         ';
      output += `${prefix}${index + 1}. ${item.className}\n`;

      if (verbose && item.packageName) {
        output += `     包: ${item.packageName}\n`;
      }

      if (showPath) {
        output += `     路径: ${item.filePath}\n`;
      }
...
// 导出单例
module.exports = new ConstantScanner();

// 独立运行测试
if (require.main === module) {
  const scanner = module.exports;

  // 测试用例
  const testDir = process.argv[2] || process.cwd();

  console.log(`正在扫描目录: ${testDir}\n`);
