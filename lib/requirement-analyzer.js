...
// 尝试加载语义推理引擎
let SemanticReasoningEngine = null;
let semanticEngine = null;

try {
  const semanticModule = require('./semantic-reasoning-engine.js');
  SemanticReasoningEngine = semanticModule.SemanticReasoningEngine;
} catch (e) {
  // 语义推理引擎不可用，降级处理
  console.warn('语义推理引擎加载失败:', e.message);
}

// 尝试加载最佳实践推荐器
let BestPracticeRecommender = null;
let practiceRecommender = null;
let practiceLoader = null;

try {
  const practiceModule = require('./best-practice-loader.js');
  BestPracticeRecommender = practiceModule.BestPracticeRecommender;
} catch (e) {
  // 最佳实践推荐器不可用，降级处理
  console.warn('最佳实践推荐器加载失败:', e.message);
}
...
class RequirementAnalyzer {
  ...
  async analyze(requirement) {
    ...
    if (SemanticReasoningEngine && !semanticEngine) {
      try {
        // 初始化语义推理引擎
        const kgModule = require('./domain-knowledge-graph.js');
        const knowledgeGraph = kgModule.loadDomainKnowledgeGraph().catch(() => null);

        const termDict = require('./terminology-dictionary.json');
        semanticEngine = new SemanticReasoningEngine(
          knowledgeGraph instanceof Promise ? null : knowledgeGraph,
          termDict
        );
      } catch (e) {
        // 忽略初始化错误
    ...
    if (semanticEngine) {
      try {
        const reasoningResult = semanticEngine.analyzeRequirement(requirement);
        implicitNeeds = reasoningResult.implicit_needs || [];
        reasoningExplanation = reasoningResult.analysis_explanation || '';
      } catch (e) {
        // 语义推理失败，使用基础分析
      }
    }
    ...
    // 最佳实践推荐（如果可用）
    let bestPractices = [];
    let sdkRecommendations = [];

    if (BestPracticeRecommender && !practiceRecommender) {
      try {
        const { BestPracticeLoader } = require('./best-practice-loader.js');
        practiceLoader = new BestPracticeLoader();
        practiceRecommender = new BestPracticeRecommender(practiceLoader);
      } catch (e) {
        // 忽略初始化错误
      }
    }

    if (practiceRecommender) {
      try {
        const recommendations = await practiceRecommender.recommend(requirement);
        bestPractices = recommendations.map(r => ({
          id: r.practice.id,
    ...
    return {
      ...
      sdk_recommendations: sdkRecommendations
    };
  }
